...
// C-linkage bridge functions for JNI access from native-lib
// native-lib uses dlsym to find these at runtime since rime is a MODULE library
extern "C" {

fcitx::rime::RimeEngine *fcitx_rime_get_engine(fcitx::Instance *instance) {
    if (!instance) return nullptr;
    auto *addon = instance->addonManager().addon("rime");
    if (!addon) return nullptr;
    return static_cast<fcitx::rime::RimeEngine *>(addon);
}

void fcitx_rime_select_schema(fcitx::Instance *instance, fcitx::InputContext *ic, const char *schema_id) {
    auto *engine = fcitx_rime_get_engine(instance);
    if (!engine || !ic) return;
    auto *state = engine->state(ic);
    if (!state) return;
    state->selectSchema(schema_id);
}

void fcitx_rime_set_ascii_mode(fcitx::Instance *instance, fcitx::InputContext *ic, bool ascii_mode) {
    auto *engine = fcitx_rime_get_engine(instance);
    if (!engine || !ic) return;
    auto *state = engine->state(ic);
    if (!state) return;
    state->setLatinMode(ascii_mode);
}

void fcitx_rime_toggle_ascii_mode(fcitx::Instance *instance, fcitx::InputContext *ic) {
    auto *engine = fcitx_rime_get_engine(instance);
    if (!engine || !ic) return;
    auto *state = engine->state(ic);
    if (!state) return;
...
bool fcitx_rime_is_ascii_mode(fcitx::Instance *instance, fcitx::InputContext *ic) {
    auto *engine = fcitx_rime_get_engine(instance);
    if (!engine || !ic) return false;
    auto *state = engine->state(ic);
...
const char *fcitx_rime_current_schema(fcitx::Instance *instance, fcitx::InputContext *ic) {
    auto *engine = fcitx_rime_get_engine(instance);
    if (!engine || !ic) return "";
    auto *state = engine->state(ic);
    if (!state) return "";
    // Return a thread-local string to avoid lifetime issues
    static thread_local std::string result;
    result = state->currentSchema();
    return result.c_str();
}
